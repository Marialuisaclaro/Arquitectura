...
class Writer:
    ...
    '''
        Escribe la seccion inicial de la ROM
    '''
    ...
    def end(self):
        ...
        while (self.linesWritten < 4096):
            print(
                '"000000000000000000000000000000000000", --{}'.format("blank"), file=self.file)
            self.linesWritten += 1
...
class Reader:
    ...
    def manageData(self, pos):
        ...
        if (len(linea) != 1):
            ...
            # MOV((Dir),A)
            self.writer.write(('MOV', '(Dir)', 'A'),
                              self.to_binary(self.variables[linea[0]]), 0)
        # Arreglos
        else:
            # MOV(A,Lit)
    ...
    def prepareWrite(self, operation, atributes):
        ...
        for j in arguments:
            ...
            # es Variable
                ...
                if j[1:-1] in self.variables:
                    valueList.append(self.to_binary(self.variables[j[1:-1]]))
                else:
                    valueList.append(self.to_binary(j[1:-1]))
        ...
        for i in self.lines:
            ...
            # Entramos a Code
                ...
                if ("RET" in i):
                    ...
                    pos = i.find(":")
                    linesWritenInitial -= 1
                    self.lineJumps[i[:pos]] = str(
                        bin(linesWritenInitial))[2:].zfill(16)
